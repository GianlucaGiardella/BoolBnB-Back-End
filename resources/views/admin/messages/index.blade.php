...
@section('contents')
    <h1 class="text-gradient m-2 ">Tutti i tuoi messaggi ricevuti</h1>

    @foreach ($apartments as $apartment)
        <div class="d-flex gap-3 align-items-center m-3 mt-5">
            <h2 class="text-gradient">{{ $apartment->title }}</h2> 
            <img src="{{asset( 'storage/'.$apartment->cover)}}" alt="{{ $apartment->title }}" style="height: 50px; border-radius: .5em;">
        </div>           
        <div class="d-flex flex-wrap gap-5 justify-content-between m-3">
            ...
            <div class="message-card">
                ...
                <!-- Button trigger modal -->
                    <button type="button" class="message-card-btn" data-bs-toggle="modal" data-bs-target="#exampleModal{{ $message->id }}">
                        Leggi il messaggio
                    </button>
                    ...
                    <div class="modal fade" id="exampleModal{{ $message->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content message-card-noMessage">
                            ...
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Da: {{ $message->email_sender}}</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div style="padding: 2em;">{{ $message->text_message }}</div>
                            <div style="font-size: 12px">Ricevuto il {{ $message->created_at->format('d/m/y') }} alle {{ $message->created_at->format('H:m') }}</div>
                        </div>
                        </div>
                    </div>
            </div>
